...
class SarasotaScraper(BaseScraper):
    ...
    def _apply_clerk_pdf_terms(
        ...
        record.owner_name = (terms.get("borrower_name", "") or record.owner_name).replace(
            "\n", " "
        ).strip()
        record.property_address = (terms.get("property_address", "") or record.property_address).replace(
            "\n", " "
        ).strip()
        record.mailing_address = (terms.get("mailing_address", "") or record.mailing_address).replace(
            "\n", " "
        ).strip()
        ...
        if credit_limit:
            ...
            estimated_principal = estimate_principal_from_doc_stamp(terms.get("doc_stamp_mortgage", ""))
        ...
        try:
            ...
            for year in self.MATURING_SEARCH_YEARS:
                ...
                for row in rows:
                    ...
                    borrower_tokens = set(
                        re.sub(r"[^A-Z0-9 ]+", " ", (borrower_name or "").upper()).split()
                    )
                    is_commercial_borrower = any(
                        keyword in borrower_tokens
                        for keyword in self.COMMERCIAL_BORROWER_KEYWORDS
                    )
                    record.notes = " | ".join(
                        [
                            part
